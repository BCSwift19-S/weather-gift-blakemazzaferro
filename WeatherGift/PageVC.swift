...
class PageVC: UIPageViewController {
    ...
    func loadLocations(){
        guard let locationsEncoded = UserDefaults.standard.value(forKey: "locationsArray") as? Data else {
            print("ERROR: Could not load locations array from user defaults")
            return
        }
        ...
        if let locationsArray = try? decoder.decode(Array.self, from: locationsEncoded) as [WeatherLocation]{
            ...
            print("ERROR: Couldn't decode locations array from user defaults")
        }
    }
    ...
    func configureListButton(){
        let safeHeight = view.frame.height - view.safeAreaInsets.bottom
        let listButton = UIButton(frame: CGRect(x: view.frame.width - barButtonWidth, y: safeHeight - barButtonHeight, width: barButtonWidth, height: barButtonHeight))
